...
export default function Index(props) {
    ...
    const isCreate = permissions.some(permission => {
        if (permission.name === 'create role') {
            return true;
        }
        return false;
    });
                                    ...
                                    {roles && roles.map((value, index) => (                                         
                                        <tr key={index}>
                                            ...
                                            {value.permissions.map((value, index) => (
                                                <p className="bg-green-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-2.5 rounded dark:bg-green-200 dark:text-blue-400 m-2 float-left capitalize">  
                                                {value.name}
                                                </p>
                                            ))}
                                            </td>
